...
def test_clone(
    ...
    original_comp = GenericSilo.at(strategy.lenders(strategy.numLenders() - 1))
    cloned_name = "Cloned_GenericSilo_" + currency.symbol()
    tx = original_comp.cloneSiloLender(
        cloned_strategy, cloned_name, xai_vault.address, {"from": gov}
    )
    cloned_lender = GenericSilo.at(tx.return_value)
    assert cloned_lender.lenderName() == cloned_name
    # Check and set staking
    cloned_strategy.addLender(cloned_lender, {"from": gov})
    with brownie.reverts():
        cloned_lender.initialize(xai_vault.address,{"from": gov})
